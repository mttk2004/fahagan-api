...
class DiscountService extends BaseService
{
    ...
    public function calculateDiscountedPrice(Book $book): float
    {
        ...
        $highestDiscount = $activeDiscounts->sortByDesc(function ($discount) use ($originalPrice) {
            if ($discount->discount_type === 'fixed') {
                // Kiểm tra điều kiện min_purchase_amount
                if ($discount->min_purchase_amount > 0 && $originalPrice < $discount->min_purchase_amount) {
                    return 0; // Không đủ điều kiện, trả về 0 để không được chọn
                }

                // Kiểm tra điều kiện max_discount_amount
