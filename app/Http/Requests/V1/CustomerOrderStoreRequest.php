...
class CustomerOrderStoreRequest extends BaseRequest implements HasValidationMessages
{
  public function rules(): array
  {
    return [
      ...
      'data.relationships.items.*.id.exists' => [
        'required',
        'integer',
        'exists:cart_items,id',
        function ($attribute, $value, $fail) {
          $cartItem = \App\Models\CartItem::find($value);
          if ($cartItem->customer_id !== AuthUtils::user()->id) {
  ...
  public function messages(): array
  {
    return [
      ...
      'data.relationships.items.*.id.exists.required' => 'ID sản phẩm là trường bắt buộc.',
      'data.relationships.items.*.id.exists.integer' => 'ID sản phẩm nên là một số nguyên.',
      'data.relationships.items.*.id.exists.exists' => 'ID sản phẩm không tồn tại trong giỏ hàng.',
