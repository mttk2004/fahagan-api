    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'relationships' => $this->when(
                $request->routeIs('orders.show', 'orders.store', 'orders.update', 'customer.orders.show', 'customer.orders.store', 'customer.orders.update'),
                [
                    'customer' => new UserResource($this->customer),
                    'employee' => new UserResource($this->employee),
                    'items' => (new OrderItemCollection($this->items))->isDirectResponse(false),
