...
class BookDTO extends BaseDTO
{
    ...
    public static function fromRequest(array $validatedData): self
    {
        ...
        $author_ids = [];
        if (isset($relationships['authors'])) {
            $author_ids = collect($relationships['authors'])->pluck('id')->toArray();
        }

        // Lấy genre_ids từ relationships nếu có
        $genre_ids = [];
        if (isset($relationships['genres'])) {
            $genre_ids = collect($relationships['genres'])->pluck('id')->toArray();
        }
        ...
        return new self(
            ...
            genre_ids: $genre_ids,
            sold_count: isset($attributes['sold_count']) ? (int) $attributes['sold_count'] : null,
            available_count: isset($attributes['available_count']) ? (int) $attributes['available_count'] : null,
        );
    }
