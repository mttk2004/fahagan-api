...
class AuthorServiceTest extends TestCase
{
  ...
  public function test_it_can_update_only_book_relationships()
  {
    ...
    // Tạo AuthorDTO chỉ cập nhật book relationships
    $authorDTO = new AuthorDTO(
      name: null,
      biography: null,
      image_url: null,
    ...
    // Kiểm tra quan hệ với sách
    $this->assertCount(1, $updatedAuthor->writtenBooks);
    $this->assertEquals($newBookIds[0], $updatedAuthor->writtenBooks[0]->id);
  }

  public function test_it_can_clear_book_relationships()
  {
    // Đảm bảo có nhà xuất bản trong database để tạo sách
    $this->seed(PublisherSeeder::class);

    // Tạo tác giả và gắn với sách
    $author = Author::factory()->create();
    $books = Book::factory()->count(3)->create();
    $author->writtenBooks()->attach($books->pluck('id'));

    // Xác nhận tác giả có 3 sách
    $this->assertCount(3, $author->writtenBooks);

    // Tạo AuthorDTO với book_ids rỗng để xóa tất cả relationships
    $authorDTO = new AuthorDTO(
      name: 'Updated Name',
      biography: null,
      image_url: null,
    ...
    // Kiểm tra tất cả relationships đã bị xóa
    $this->assertCount(0, $updatedAuthor->writtenBooks);
  }
