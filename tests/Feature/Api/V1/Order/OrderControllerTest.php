...
class OrderControllerTest extends TestCase
{
  ...
  #[Test]
  public function employee_can_update_order_status_from_approved_to_delivering()
  {
    // Đảm bảo người dùng có quyền chỉnh sửa đơn hàng
    $this->employee->givePermissionTo('edit_orders');
    Sanctum::actingAs($this->employee);
    ...
    $this->assertEquals(OrderStatus::DELIVERING->value, $this->order->status);
    $this->assertNotNull($this->order->delivering_at, 'delivering_at chưa được cập nhật trong database');
  }

  #[Test]
  public function employee_can_update_order_status_from_delivering_to_delivered()
  {
    // Đảm bảo người dùng có quyền chỉnh sửa đơn hàng
    $this->employee->givePermissionTo('edit_orders');
    Sanctum::actingAs($this->employee);

    // Đầu tiên, chuyển trạng thái đơn hàng sang DELIVERING
    $this->order->status = OrderStatus::DELIVERING->value;
    $this->order->employee_id = $this->employee->id;
    $this->order->delivering_at = now();
    $this->order->save();

    // Gọi API để cập nhật trạng thái đơn hàng từ DELIVERING sang DELIVERED
    $response = $this->patchJson("/api/v1/orders/{$this->order->id}/status", [
